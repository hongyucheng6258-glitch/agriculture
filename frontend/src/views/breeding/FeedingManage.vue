    ...
    <el-card shadow="never" class="search-card">
      <el-form :inline="true" :model="searchForm" class="search-form">
        ...
        <el-form-item label="饲料类型">
          <el-select v-model="searchForm.feedType" placeholder="请选择饲料类型" clearable @change="handleSearch">
            <el-option
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" width="520px" @close="resetForm">
      <el-form ref="feedingForm" :model="feedingForm" :rules="feedingRules" label-width="100px">
        ...
        <el-form-item label="投喂量(kg)" prop="feedAmount">
          <el-input-number v-model="feedingForm.feedAmount" :min="0" :precision="2" :step="0.5" style="width: 100%;" />
        </el-form-item>
...
export default {
  ...
  created() {
    this.getList();
    this.getFeedStockList();
    this.$store.dispatch('fetchCageList');
  },
  methods: {
    async getFeedStockList() {
      try {
        const res = await getFeedStockList();
        this.feedStockList = res.data || [];
      } catch (error) {
